...
const EXPIRATION_CLAIM_KEYS = ['exp', 'expiration', 'expires', 'expiresAt']
...
function extractExpiration(claims: ClaimRecord | null): number | null {
  if (!claims) {
    return null
  }
  for (const key of EXPIRATION_CLAIM_KEYS) {
...
function isTokenExpired(token: string | null): boolean {
  ...
  return isExpired(extractExpiration(claims))
}
